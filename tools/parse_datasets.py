...
APP_JS = ROOT / "app.js"
QUESTIONS_JSON = ROOT / "questions.json"  # runtime-loadable bank served with the app
...
# CATEGORIES constant name -> the human-readable value used by the app at runtime
CATEGORY_VALUE = {
    "SECURITY": "Security & Access",
    "AUTOMATION": "Automation & Logic",
    "DATA": "Data Management",
    "APPS": "Sales & Service Apps",
    "UI": "UI & Analytics",
    "DEPLOY": "Deployment & Sandboxes",
}
...
def main():
    ...
    # 1) The runtime-loadable bank: a plain JSON the app fetches at startup, so
    #    the question bank (including explanations + hooks) can be swapped without
    #    touching app.js. `category` is resolved to its human-readable value.
    runtime = [
        {
            "id": q["id"],
            "category": CATEGORY_VALUE.get(q["category"]),
            "text": q["text"],
            "options": q["options"],
            "answer": q["answer"],
            "multi": q["multi"],
    ...
    QUESTIONS_JSON.write_text(
        json.dumps(runtime, indent=1, ensure_ascii=False), encoding="utf-8"
    )

    # 2) The embedded fallback bank inside app.js (used when questions.json can't
    #    be fetched, e.g. opening index.html from file://). Declared with `let`
    #    so the runtime fetch can replace it.
    block = (
    ...
    print(f"Wrote runtime bank to {QUESTIONS_JSON.name}")
