// ============================================================
// Salesforce Platform Administrator II Exam Simulator (PLAT-ADMN-301)
...
const TAB_CATEGORIES = [
  ...
  CATEGORIES.UI,
  CATEGORIES.DEPLOY,
  null  // Tab 7 = Full Exam Simulator
];

const FULL_EXAM_TAB = 7;
...
const SESSION_STORAGE_KEY = "plat_admn_301_session";
...
const KNOCKOUT_STORAGE_KEY = "plat_admn_301_knockout";
...
function saveKnockout() {
  ...
  try {
    localStorage.setItem(KNOCKOUT_STORAGE_KEY, JSON.stringify({
      version: 1,
...
function loadKnockout() {
  try {
    const raw = localStorage.getItem(KNOCKOUT_STORAGE_KEY);
    if (!raw) return null;
...
function clearKnockout() {
  try {
    localStorage.removeItem(KNOCKOUT_STORAGE_KEY);
  } catch (e) {
    // ignore
  }
  updateRapidResumeHint();
...
async function loadBank() {
  try {
    ...
    if (!valid) {
      console.warn("questions.json failed validation — using the embedded bank.");
      return null;
    }
    return data;
  } catch (e) {
    // file:// or offline — fall back to the embedded bank
    return null;
  }
}
...
function renderTabButtons() {
  ...
  const tabs = [
    ...
    { label: "Sales & Service Apps" },
    { label: "UI & Analytics" },
    { label: "Deployment & Sandboxes" },
    { label: `Full Exam (${QUESTIONS.length})`, icon: "🎯" }
  ];
...
function renderDashboard() {
  ...
  const topicList = Object.values(CATEGORIES).map(cat => ({
    name: cat,
  ...
  pane.innerHTML = `
    <div class="dashboard-grid">
      <div class="dash-card">
        <h2>📋 Exam Overview</h2>
        <p>Salesforce Certified Platform Administrator II (PLAT-ADMN-301)</p>
        <div class="stat-row">
          <div class="stat-box">
          ...
          <div class="stat-box">
            <div class="stat-value">${EXAM_MINUTES}</div>
            <div class="stat-label">Minutes</div>
          </div>
          <div class="stat-box">
            <div class="stat-value">${PASSING_SCORE}%</div>
...
function updateScoreMatrix() {
  ...
  const passing = parseFloat(score) >= PASSING_SCORE;
...
function toggleMode(isExam) {
  ...
  if (isExam) {
    ...
    state.timerSeconds = EXAM_MINUTES * 60;
...
function submitExam() {
  ...
  const passed = parseFloat(score) >= PASSING_SCORE;
...
function showResultsModal(answered, correct, score, passed, sections) {
  ...
  modal.innerHTML = `
    ...
    <p class="result-subtitle">${passed ? "You passed the practice exam!" : `You didn't reach the ${PASSING_SCORE}% passing threshold.`}</p>
    ...
    <div class="results-breakdown">
      ...
      <div class="result-stat ${passed ? 'pass' : 'fail'}">
        ...
        <div class="rs-label">Status (≥${PASSING_SCORE}%)</div>
      </div>
    </div>
      ...
      <tbody>
        ${Object.entries(sections).filter(([, data]) => data.total > 0).map(([name, data]) => {
          ...
          return `<tr>
            ...
            <td style="color:${pct >= PASSING_SCORE ? 'var(--color-correct)' : 'var(--color-incorrect)'}">${pct >= PASSING_SCORE ? "✓ Pass" : "✕ Needs Work"}</td>
          </tr>`;
        }).join("")}
      </tbody>
    </table>
